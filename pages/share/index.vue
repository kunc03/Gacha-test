<template>
  ...
  >
    ...
    <section class="min-h-dvh max-h-[calc(100dvh-30px)] overflow-y-auto pb-6" v-if="shareDetailData == null">
    ...
    <section class="min-h-dvh max-h-[calc(100dvh-30px)] overflow-y-auto pb-6" v-else>
      <div class="flex flex-col bg-center text-black mt-[105px] px-8 gap-3">
        <div class="max-w-sm bg-white rounded-lg shadow">
          <div class="h-72 w-full overflow-hidden rounded-t-lg">
            <Skeleton v-if="isFetching" class="!w-full !h-full" />
            <CharacterCard v-else-if="shareDetailData.is_valid_char" :image="shareDetailData.character_image" />
          </div>
